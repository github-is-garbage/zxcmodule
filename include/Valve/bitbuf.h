...
namespace bitbuf
{
	...
	inline unsigned long long ZigZagEncode64(long long n)
	{
		// Note:  the right-shift must be arithmetic
		return(n << 1) ^ (n >> 63);
	}

	inline long long ZigZagDecode64(unsigned long long n)
	{
...
public:
	...
	void			WriteVarInt32( unsigned int data );
	void			WriteVarInt64( unsigned long long data );
	void			WriteSignedVarInt32( int data );
	void			WriteSignedVarInt64( long long data );
	int				ByteSizeVarInt32( unsigned int data );
	int				ByteSizeVarInt64( unsigned long long data );
	int				ByteSizeSignedVarInt32( int data );
	int				ByteSizeSignedVarInt64( long long data );
...
{
	union { unsigned int u; float f; } c = { ReadUBitLong(32) };
	return c.f;
}
...
inline unsigned int bf_read::ReadUBitLong( int numbits ) __restrict
{
	...
	unsigned int dw1 = ((unsigned long* __restrict)m_pData)[iWordOffset1] >> iStartBit;
	unsigned int dw2 = ((unsigned long* __restrict)m_pData)[iWordOffset2] << (32 - iStartBit);

...
#include <Valve/bitvec.h>
#include <Valve/coordsize.h>
...
void bf_write::WriteVarInt32( unsigned int data )
{
	// Check if align and we have room, slow path if not
	if ( (m_iCurBit & 7) == 0 && (m_iCurBit + bitbuf::kMaxVarint32Bytes * 8 ) <= m_nDataBits)
	{
		unsigned char *target = ((unsigned char*)m_pData) + (m_iCurBit>>3);

		target[0] = static_cast<unsigned char>(data | 0x80);
		if ( data >= (1 << 7) )
		{
			target[1] = static_cast<unsigned char>((data >>  7) | 0x80);
			if ( data >= (1 << 14) )
			{
				target[2] = static_cast<unsigned char>((data >> 14) | 0x80);
				if ( data >= (1 << 21) )
				{
					target[3] = static_cast<unsigned char>((data >> 21) | 0x80);
					if ( data >= (1 << 28) )
					{
						target[4] = static_cast<unsigned char>(data >> 28);
						m_iCurBit += 5 * 8;
						return;
					}
					else
...
void bf_write::WriteVarInt64( unsigned long long data )
{
	// Check if align and we have room, slow path if not
	if ( (m_iCurBit & 7) == 0 && (m_iCurBit + bitbuf::kMaxVarintBytes * 8 ) <= m_nDataBits )
	{
		...
		// processors.
		unsigned int part0 = static_cast<unsigned int>(data      );
		unsigned int part1 = static_cast<unsigned int>(data >> 28);
		unsigned int part2 = static_cast<unsigned int>(data >> 56);
...
void bf_write::WriteSignedVarInt32( int data )
{
	WriteVarInt32( bitbuf::ZigZagEncode32( data ) );
}

void bf_write::WriteSignedVarInt64( long long data )
{
	WriteVarInt64( bitbuf::ZigZagEncode64( data ) );
}

int	bf_write::ByteSizeVarInt32( unsigned int data )
{
	int size = 1;
	while ( data > 0x7F ) {
		size++;
...
int	bf_write::ByteSizeVarInt64( unsigned long long data )
{
	int size = 1;
	while ( data > 0x7F ) {
		size++;
...
int bf_write::ByteSizeSignedVarInt32( int data )
{
	return ByteSizeVarInt32( bitbuf::ZigZagEncode32( data ) );
}

int bf_write::ByteSizeSignedVarInt64( long long data )
{
...
bool bf_write::WriteBits(const void *pInData, int nBits)
{
	...
	if ( true && (nBitsLeft >= 32) && (m_iCurBit & 7) == 0 )
	{
...
void bf_write::WriteLongLong(long long val)
{
	unsigned int *pLongs = (unsigned int*)&val;

	// Insert the two DWORDS according to network endian
	const short endianIndex = 0x0100;
	unsigned char *idx = (unsigned char*)&endianIndex;
...
unsigned int bf_read::ReadVarInt32()
{
...
unsigned long long bf_read::ReadVarInt64()
{
	...
	do
	{
		...
		b = ReadUBitLong( 8 );
		result |= static_cast<unsigned long long>(b & 0x7F) << (7 * count);
		++count;
	} while (b & 0x80);
...
int bf_read::ReadSignedVarInt32()
{
	unsigned int value = ReadVarInt32();
	return bitbuf::ZigZagDecode32( value );
}

long long bf_read::ReadSignedVarInt64()
{
	unsigned int value = ReadVarInt64();
...
long long bf_read::ReadLongLong()
{
	long long retval;
...
int bf_read::CompareBitsAt( int offset, bf_read * __restrict other, int otherOffset, int numbits ) __restrict
{
	...
	while ( numbits > 32 )
	{
		x  = ((unsigned long*)pData1)[0] >> iStartBit1;
		x ^= ((unsigned long*)pData1)[1] << (32 - iStartBit1);
		x ^= ((unsigned long*)pData2)[0] >> iStartBit2;
		x ^= ((unsigned long*)pData2)[1] << (32 - iStartBit2);
		if ( x != 0 )
		{
			return x;
		}
	...
	x  = ((unsigned long*)pData1)[0] >> iStartBit1;
	x ^= ((unsigned long*)pData1End)[0] << (32 - iStartBit1);
	x ^= ((unsigned long*)pData2)[0] >> iStartBit2;
	x ^= ((unsigned long*)pData2End)[0] << (32 - iStartBit2);
